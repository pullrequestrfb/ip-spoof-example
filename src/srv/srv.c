...
static void on_alloc(uv_handle_t *client, size_t suggest_size, uv_buf_t *buf)
{
...
int listen_and_serve(server_t *s, uv_loop_t *loop)
{
    ...
    uv_ip4_addr(s->addr_str, s->port, s->socket);
    int status = uv_udp_bind(s->socket, (const struct sockaddr*)&srv_addr, UV_UDP_REUSE_ADDR);
    if(status != 0)
    {
...
{
    uv_udp_send_t req;
    uv_buf_t buf = buf_init(payload, sizeof(payload));
    return uv_udp_send(&req, s->socket, &buf, s->addr, on_send);
}
